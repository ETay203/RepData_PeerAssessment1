...
```
The mean number of steps is `r mean_steps`. The median number of steps is `r median_steps`.
...
```
The 5-minute interval, on average across all the days in the dataset, that contains the maximum number of steps is `r most_steps$Interval`.
...
The mean and median number of steps per day in the filled data are `r mean_stepsFill` and `r median_stepsFill`, respectively. 
...
On weekdays there is large peak of activity relatively early in the day at the `r most_wkday$Interval` 5-minute interval. After which the average number of steps taken drops significantly.  The mean of the average number of steps taken (across all days) is `r mean_wkday`.  For weekends there are a number of peaks throughout the day, with overall a slightly higher mean of `r mean_wkend`.
